...
async def test_fetch_user_list__with_users__status(
...
async def test_fetch_user_list__with_users__format(
    client: AsyncClient, create_db_user_factory
):
    user1 = await create_db_user_factory(id=UUID_1)
    user2 = await create_db_user_factory(id=UUID_2)
    response = await client.get(API_URL)
...
async def test_fetch_user_list__with_limit(client: AsyncClient, create_db_user_factory):
    user1 = await create_db_user_factory(id=UUID_1)
    await create_db_user_factory(id=UUID_2)
...
async def test_fetch_user_list__with_offset(
    client: AsyncClient, create_db_user_factory
):
    await create_db_user_factory(id=UUID_1)
    user2 = await create_db_user_factory(id=UUID_2)
